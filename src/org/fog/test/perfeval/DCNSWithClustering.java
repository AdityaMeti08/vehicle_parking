 ...
 *
 */
public class DCNSWithClustering {
	...
	public static void main(String[] args) {
		...
		try {
			...
			createClusteredFogDevices(broker.getId(), appId);
			...
			for (FogDevice device : fogDevices) {
                device.setUplinkLatency(device.getUplinkLatency() + random.nextDouble() * 100- 30); // Adjust the range accordingly
            }
	 ...
	 * @param appId
	 */
	private static void createClusteredFogDevices(int userId, String appId) {
		FogDevice cloud = createFogDevice("cloud", 44800, 40000, 100, 10000, 0, 0.01, 16*103, 16*83.25);
		cloud.setParentId(-1);
		int numOfClusters = 10; // Set your desired number of clusters
        List<FogDevice> clusters = new ArrayList<>();
		fogDevices.add(cloud);
		for (int i = 0; i < numOfClusters; i++) {
            FogDevice cluster = createCluster("cluster-" + i, 10000, 8000, 100000, 100000, 2, 0.01, 30, 20);
            clusters.add(cluster);
        }
		FogDevice proxy = createFogDevice("proxy-server", 2800, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
		proxy.setParentId(cloud.getId());
		proxy.setUplinkLatency(10); // latency of connection between proxy server and cloud is 100 ms
		 for (FogDevice cluster : clusters) {
	            cluster.setParentId(proxy.getId());
	        }
	...
	private static FogDevice createCluster(String nodeName, long mips,
            int ram, long upBw, long downBw, int level, double ratePerMips, double busyPower, double idlePower) {
		List<Pe> peList = new ArrayList<Pe>();
		FogDevice cluster = createFogDevice(nodeName, mips, ram, upBw, downBw, level, ratePerMips, busyPower, idlePower);
