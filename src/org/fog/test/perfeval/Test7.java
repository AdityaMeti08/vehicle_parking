...
import org.fog.scheduler.KMeansClustering;
...
public class Test7 {
	public class KMeansScheduler {
	    private static List<List<FogDevice>> clusters;
	    private static int currentClusterIndex = 0;
	    private static int currentDeviceIndex = 0;

	    public KMeansScheduler(List<List<FogDevice>> clusters, int currentClusterIndex2, int currentDeviceIndex2) {
	        this.clusters = clusters;
	    }
	    public String getNextDeviceName() {
	        if (!clusters.isEmpty()) {
	            List<FogDevice> currentCluster = clusters.get(currentClusterIndex);
	            FogDevice currentDevice = currentCluster.get(currentDeviceIndex);

	            // Move to the next device in the current cluster in a circular manner
	            currentDeviceIndex = (currentDeviceIndex + 1) % currentCluster.size();

	            // If all devices in the current cluster have been visited, move to the next cluster
	            if (currentDeviceIndex == 0) {
	                currentClusterIndex = (currentClusterIndex + 1) % clusters.size();
	            }

	            return currentDevice.getName();
	        }
	        return null; // Return null if there are no clusters
	    }
	...
	static int numofVehicles=5;
	static int numofAmbulances=1;
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		...
		for(int i=0;i<numofVehicles;i++){
			...
			FogDevice vehicle = addVehicle(mobileId, userId, appId, router.getId()); // adding a smart camera to the physical topology. Smart cameras have been modeled as fog devices as well.
			vehicle.setUplinkLatency(2); // latency of connection between camera and router is 2 ms
	...
	public static void main(String[] args) {
		...
		try {
			...
			KMeansClustering kMeansClustering = new KMeansClustering(fogDevices,3);
			int num_user = 1; // number of cloud users
			Calendar calendar = Calendar.getInstance();
			boolean trace_flag = false; // mean trace events
			List<List<FogDevice>> clusters = new ArrayList<>(); // You need to initialize your clusters
            int currentClusterIndex = 0;
            int currentDeviceIndex = 0;
            List<FogDevice> updatedDevices = new ArrayList<>(); // You need to initialize your updatedDevices
            kMeansClustering.setDevices(updatedDevices);
            String nextDeviceName = kMeansClustering.getNextDeviceName();

            // Create an instance of the outer class
            Test7 test7 = new Test7();

            // Create an instance of the inner class KMeansScheduler
            KMeansScheduler kMeansScheduler = test7.new KMeansScheduler(clusters, currentDeviceIndex, currentDeviceIndex);
	...
	private static FogDevice addVehicle(String id, int userId, String appId, int parentId){
		...
		actuators.add(ptz);
		sensor.setGatewayDeviceId(vehicle.getId());
		sensor.setLatency(1.0);  // latency of connection between camera (sensor) and the parent Smart Camera is 1 ms
		ptz.setGatewayDeviceId(vehicle.getId());
		ptz.setLatency(1.0);  // latency of connection between PTZ Control and the parent Smart Camera is 1 ms
		return vehicle;
	}

	
	abstract class ModulePlacementCentralized extends ModulePlacement {
	    private KMeansScheduler scheduler;
	    private ModuleMapping moduleMapping;

	    public ModulePlacementCentralized(List<FogDevice> fogDevices, KMeansScheduler scheduler) {
	        super();
	        this.scheduler = scheduler;
	        this.moduleMapping = ModuleMapping.createModuleMapping();
	    }
	    ...
	    public void mapModules(Application application) {
	        List<org.fog.application.AppModule> modules = application.getModules();
	        for (org.fog.application.AppModule module : modules) {
	            String deviceName = scheduler.getNextDeviceName(); // Get the next device name from the scheduler
	            this.moduleMapping.addModuleToDevice(module.getName(), deviceName);
	        }
	    }
	}
